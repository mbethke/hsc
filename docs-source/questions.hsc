<DOCPAGE chapter="hsc - " title="Questions And Answers"
    PREV="usecases.html"
    NEXT="source.html"
...
<$macro Q.pipe>I tried a <KBD>STATUS=line|verbose</KBD> on the commandline, and now can't find the <EXEC>verbose</EXEC> command?!</$macro>
...
<ANSWR>The <vbar> is commonly used in several CLIs to <qq>pipe</qq> the output of
...
in linked lists. And therefore, if <hsc> has to look for something, it
...
perpetrated by some braindead Unix fossils, ignoring the fact that
memory can run out or that there is a <qq>-Wall</qq> option in most
compilers; some of these sources also date back to 1863, when no one
even pretended there was a language definition for C.</p>
...
<ANSWR>Jerks are persons suffering by a well-known epidemic called
<EM>Netscapism</EM><SUP><a href="#ftn1">1</a></SUP>. Their sources contain sections like
<PRE>
    <TG>BODY BGCOLOR=#123456</TG>
    <TG>BLINK</TG>Click here!<TG>/BLINK</TG>
</PRE>
<P>But, not all jerks became jerks because they like to be jerks. Very often,
jerks are forced to use code as seen above by their employer. In this case,
they should be refered to as <qq>prostitutes</qq>.</P>
<P><a name="ftn1"></a>(1) Or that's what it was called in the Dark Ages of the Web when Agi
first wrote this. Now, in the Even Darker Though Truecolor Ages, it's mostly a
similar disease called Exploritis. However, owing to HSC's lack of colorful
buttons, <qq>wizards</qq> and the like, HSC users and people suffering from these
diseases should be fairly disjunct sets. --mb</P>
</DOCPAGE>
