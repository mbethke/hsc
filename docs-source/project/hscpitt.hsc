...
<pittCmd NAME="ADD">
<P>Setting <KBD>COMMAND=ADD</KBD> will add a new document with
it's corresponding main source to the project. The first
<CODE>ARG</CODE> contains the <EM>full</EM> relative filename
of the html object, the second <CODE>ARG</CODE> contains the
name of the main hsc-source that will be used to create the
document.</P>

If the document already is part of the project, an error message
will be displayed and the old document will be left untouched.
To replace a document, you will have to
<A HREF="#DELETE"><KBD>DELETE</KBD></A> it before.
