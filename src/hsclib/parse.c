                ...
                if ( tag->option & (HT_CLOSE | HT_AUTOCLOSE) ) {
                    ...
                    */
                    ...
                    hsc_message( hp, MSG_ILLG_CTAG,   /* tag not found */
                                 "illegal end tag %c", nxtwd );

                    skip_until_eot( hp, hp->tag_attr_str ); /* TODO: parse for ">" only */
...
#if 0
            outstr( hp, estr2str( hp->tag_name_str ) );  /* output  "<tagname " */
            outstr( hp, estr2str( hp->tag_attr_str ) );  /* output attributes */
            outstr( hp, estr2str( hp->tag_close_str ) ); /* output ">" */
#endif
        }
            ...
            if ( !strcmp( nxtwd, "#" ) ) {
                ...
                */
                ...
                if ( errno || strlen( infgetcws( inpf ) ) ) {
                    ...
                    hsc_message( hp, MSG_ILLEGAL_NUM, /* illegal numeric entity */
                                 "illegal %n", nxtwd );
...
BOOL hsc_parse_end_id( HSCPRC *hp )
{
    if ( !hp->fatal ) {
        ...
        /* check local IDs */
        check_all_local_id_ref( hp );

#if 0 /* TODO: remove */
        if ( hp->filename_process && hp->filename_project ) {
            hsc_write_project_file( hp );
        } else {
            D( fprintf( stderr, DHL "no update project\n" ) );
        }
#endif
