...
*/
static BOOL check_reqvar( HSCPRC *hp, HSCVAR *var )
{
    BOOL    ok   = TRUE;
...
BOOL check_varlist( HSCPRC *hp, DLLIST *varlist )
{
    ...
    while ( nd ) {
        ...
        ok &= check_reqvar( hp, (HSCVAR *) (nd->data) );
        nd = nd->next;
...
*/
static BOOL read_enum_str( HSCPRC *hp, HSCVAR *var )
{
    ...
    ok = parse_wd( hp, "(" );        /* check for "(" */
    ok &= clr_estr( hp->tmpstr );          /* reset string */

    ch = infgetc( inpf );
    while ( ok && (ch!=')') && (ch!=EOF) && (ch!='\n') ) {

        if ( (ch!=')') && (ch!=' ') )
            ok &= app_estrch( hp->tmpstr, ch );
        ch = infgetc( inpf );
    ...
    if ( ok )
    if ( ch == EOF )
        hsc_msg_eof( hp, "reading enumerator" );
    else if ( ch == '\n' )
        hsc_msg_eol( hp );
    ...
    if ( ok ) {
        ...
        var->enumstr = strclone( estr2str( hp->tmpstr ) );

    }
#endif
    HSCATTR *attr = new_hscattr( "_dummy_enum" );
...
*/
static BOOL check_attr_option( HSCPRC *hp, STRPTR option, HSCVAR *attr, STRPTR id, STRPTR sid, ULONG value, ULONG unmasked_flags )
{
    BOOL found = FALSE;
...
*/
HSCVAR *define_var( HSCPRC *hp, DLLIST *varlist, ULONG unmasked_flags )
{
    ...
    while ( nw && !strcmp( nw, "/" ) ) {
        ...
        if ( nw ) {
            ...
            ok |= check_attr_option( hp, nw, var,
                      VF_CONST_STR, VF_CONST_SHT, VF_CONST, unmasked_flags );
            ok |= check_attr_option( hp, nw, var,
                      VF_GLOBAL_STR, VF_GLOBAL_SHT, VF_GLOBAL, unmasked_flags );
            ok |= check_attr_option( hp, nw, var,
                      VF_JERK_STR, VF_JERK_SHT, VF_JERK, unmasked_flags );
            ok |= check_attr_option( hp, nw, var,
                      VF_ONLYONCE_STR, VF_ONLYONCE_SHT, VF_ONLYONCE, unmasked_flags );
            ok |= check_attr_option( hp, nw, var,
                      VF_REQUIRED_STR, VF_REQUIRED_SHT, VF_REQUIRED, unmasked_flags );
            ok |= check_attr_option( hp, nw, var,
                      VF_GETSIZE_STR, VF_GETSIZE_SHT, VF_GETSIZE, unmasked_flags );
            ok |= check_attr_option( hp, nw, var,
                      VF_STRIPEXT_STR, VF_STRIPEXT_SHT, VF_STRIPEXT, unmasked_flags );
            if ( !ok ) {
            ...
            nw = infgetw( inpf );      /* read next word (should be "/", */
                                       /*   "=" or next attr / ">") */
        } else
    ...
    */
    if ( nw && !strcmp( nw, "=" ) ) {
        ...
        if ( var->vartype == VT_NUM ) {

            /* TODO: test-set value with default value */
            /* TODO: check numeric value */
            LONG num;

            if ( (size_t)sscanf( var->text, "%ld", &num ) != strlen(var->text) ) {

                ok = FALSE;
                hsc_message( hp, MSG_ILLEGAL_NUM,
                             "illegal numeric value: %q", var->text );
...
*/
static HSCVAR *copy_local_var( DLLIST *destlist, HSCVAR *locvar, ULONG mci )
{
...
*/
static HSCVAR *set_local_var( DLLIST *destlist, HSCVAR *locvar, ULONG mci )
{
