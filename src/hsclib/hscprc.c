...
VOID del_hscprc( HSCPRC *hp )
{
    if ( hp ) {
        ...
        del_dllist( hp->id_global );
        del_dllist( hp->id_local );
        del_dllist( hp->id_ref );
        del_dllist( hp->subdoc );
...
HSCPRC *new_hscprc( void )
{
    ...
    if ( hp ) {
        ...
        hp->id_global       = init_dllist( del_id_global );
        hp->id_local        = init_dllist( del_id_local );
        hp->id_ref          = init_dllist( del_id_ref );
        hp->subdoc          = init_dllist( del_subdoc );
...
STRPTR hsc_get_last_anchor( HSCPRC *hp )
{
    return( hp->last_anchor );
}
...
BOOL hsc_set_filename_process( HSCPRC *hp, STRPTR filename )
{
    ...
    if ( !hp->filename_process ) {
        ...
        hp->filename_process = strclone( filename );
        ok = TRUE;
...
** set call-backs
*/
VOID hsc_set_status_file_begin( HSCPRC *hp, VOID (*status_file)(HSCPRC *hp) )
{
...
** nomem-handler
*/
BOOL hsc_standard_nomem_handler( size_t size ) /* TODO: think about this */
{
    fputs( "\n*** out of memory\n\n", stderr );
